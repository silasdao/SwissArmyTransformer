...
try:
    ...
    print(f"{WARNING} unable to import torch, please install it if you want to pre-compile any sat ops.")
else:
...
def assert_no_cuda_mismatch(name=""):
    ...
    if sys_cuda_version != torch_cuda_version:
        if (cuda_major in cuda_minor_mismatch_ok and sys_cuda_version in cuda_minor_mismatch_ok[cuda_major]
            ...
            return True
        raise Exception(f">- sat Op Builder: Installed CUDA version {sys_cuda_version} does not match the "
...
class OpBuilder(ABC):
    ...
    def validate_torch_version(torch_info):
        ...
        if install_torch_version != current_torch_version:
            raise RuntimeError("PyTorch version mismatch! SAT ops were compiled and installed "
    ...
    def validate_torch_op_version(torch_info):
        if not OpBuilder.is_rocm_pytorch():
            ...
            if install_cuda_version != current_cuda_version:
                raise RuntimeError("CUDA version mismatch! SAT ops were compiled and installed "
                                   "with a different version than what is being used at runtime. "
                                   f"Please re-install SAT or switch torch versions. "
            ...
            if install_hip_version != current_hip_version:
                raise RuntimeError("HIP version mismatch! SAT ops were compiled and installed "
                                   "with a different version than what is being used at runtime. "
                                   f"Please re-install SAT or switch torch versions. "
    ...
    def load(self, verbose=True):
        ...
        if not installed_ops[self.name]:
...
class CUDAOpBuilder(OpBuilder):
    ...
    def builder(self):
        ...
        if self.is_rocm_pytorch():
            ...
            for i in range(len(sources)):
                src = Path(sources[i])
                sources[i] = str(src.relative_to(curr_file)) if src.is_absolute() else str(src)
    ...
    def libraries_args(self):
        ...
        return ['cublas', 'curand'] if sys.platform == "win32" else []
...
class TorchCPUOpBuilder(CUDAOpBuilder):
    ...
    def extra_ldflags(self):
        ...
        return ['-lcurand'] if not self.is_rocm_pytorch() else []
