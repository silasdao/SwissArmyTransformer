...
def transform_weight(hugging_model, swiss_model):
    ...
    for src_l, dst_l, w2 in zip(hugging_model.transformer.encoder.layers, swiss_model.transformer.layers, swiss_model.mixins['mlp'].w2):
...
with torch.no_grad():
    ...
    dst_output = model.cuda()(input_ids=encoded_input['input_ids'], position_ids=encoded_input['position_ids'], attention_mask=torch.ones(1, 1, 2, 2, dtype=torch.float16, device='cuda').tril())
