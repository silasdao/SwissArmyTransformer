...
def create_dataset_function(path, args):
    def process_fn(row):
        pack, label = _encode(row['paragraph'], row['question'] + ' ' + row['answer']), int(row['label'])
        return {
    ...
    return load_hf_dataset(path, process_fn, columns = ["input_ids", "position_ids", "attention_mask", "label"], cache_dir='/dataset/fd5061f6/satDatasets', offline=False, transformer_name="multirc_transformer")
